    ...
    public class SDKHandler : IDisposable
    {
        ...
        public void OpenSession(Camera newCamera)
        {
            ...
            if (newCamera != null)
            {
                ...
                uint sdk_error_event = EDSDK.EdsSetCameraStateEventHandler(MainCamera.Ref, EDSDK.StateEvent_All, SDKStateEvent, IntPtr.Zero);
                uint sdk_error_set = EDSDK.EdsSetObjectEventHandler(MainCamera.Ref, EDSDK.ObjectEvent_All, SDKObjectEvent, IntPtr.Zero);
                uint sdk_error_property = EDSDK.EdsSetPropertyEventHandler(MainCamera.Ref, EDSDK.PropertyEvent_All, SDKPropertyEvent, IntPtr.Zero);
        ...
        private void DownloadEvf()
        {
            LVThread = new Thread(delegate()
            {
                ...
                uint length;
                //create stream
                lock (LVlock) Error = EDSDK.EdsCreateMemoryStream(0, out stream);
        ...
        public void TakePhoto()
        {
            ...
            lock (LVlock) { Error = EDSDK.EdsSendCommand(MainCamera.Ref, EDSDK.CameraCommand_TakePicture, 0); }
        }
